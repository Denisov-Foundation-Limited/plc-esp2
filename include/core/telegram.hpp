...
public:
    ...
    PollState pollTick(std::vector<Update> &out)
    {
        ...
        {
            String path = "/bot" + _token + "/getUpdates?timeout=" + String(_poll_timeout_s);
            if (_poll_offset > 0)
                path += "&offset=" + String(_poll_offset);
            if (_poll_limit > 0)
                path += "&limit=" + String(_poll_limit);

            _client.print(F("GET "));
            _client.print(path);
            _client.println(F(" HTTP/1.1"));
            _client.println(F("Host: api.telegram.org"));

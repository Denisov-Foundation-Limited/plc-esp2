...
public:
    ...
    void setArmStateHandlerSecondary(ArmStateHandler cb, void *ctx);
    void setPreArmCheckHandler(PreArmCheckHandler cb, void *ctx);
    void setAlarmStateHandler(AlarmStateHandler cb, void *ctx);
    void setAlarmStateHandlerSecondary(AlarmStateHandler cb, void *ctx);
    void setClearDetectHandler(ClearDetectHandler cb, void *ctx);
    void setClearDetectHandlerSecondary(ClearDetectHandler cb, void *ctx);
    ...
    size_t prearmTriggeredCount();
    void notifyRemoteDetect(uint32_t node_id, const String &source, uint8_t sensor_id, const String &name, bool silent);
    size_t remoteDetectCount(uint32_t node_id = 0) const;
    bool remoteDetectAt(size_t idx, RemoteDetect &out, uint32_t node_id = 0) const;
    void clearRemoteDetects(uint32_t node_id = 0);
...
private:
    ...
    void setSensorPortDebounce_(uint8_t port, bool enabled);
    bool readRaw_(const SensorConfig &cfg, bool &out);
    static bool isTriggered_(const SensorConfig &cfg, bool raw);
    static uint32_t debounceMs_(const SensorConfig &cfg);
    bool sampleTriggered_(const SensorConfig &cfg, SensorState &st, uint32_t now_ms);
    void initIButton_();

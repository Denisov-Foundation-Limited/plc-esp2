...
private:
    ...
    void reset_();static bool indexById_(uint8_t id, size_t &out);void setupInputs_(const SepticConfig &cfg);void setupInput_(uint8_t port);void setupOutputs_(const SepticConfig &cfg, SepticState &st);void setupRelay_(uint8_t port, bool &state);void readLevels_(const SepticConfig &cfg, SepticState &st);bool setLevelPort_(size_t id, uint8_t port, uint8_t SepticConfig::*field);bool setRelayPort_(size_t id, uint8_t port, uint8_t SepticConfig::*field);bool readInput_(uint8_t port, bool &out);void updateRelays_(const SepticConfig &cfg, SepticState &st);void writeRelay_(uint8_t port, bool on);void logLevelChange_(const SepticConfig &cfg, const SepticState &prev, const SepticState &curr);void logRelayChange_(const SepticConfig &cfg, const SepticState &prev, const SepticState &curr);void notifyLevel_(const SepticConfig &cfg, bool is_alarm);void notifyDetectEvent_(const SepticConfig &cfg, bool is_alarm);static bool parsePort_(JsonVariantConst v, uint8_t &out);static constexpr bool kRelayInvert = false;

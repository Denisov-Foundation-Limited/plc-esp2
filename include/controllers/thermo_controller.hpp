...
public:
    ...
    using EventHandler = void (*)(void *ctx, uint8_t id, const String &name, bool power_on, const char *source);
...
private:
    ...
    void reset_();bool indexById_(uint8_t id, size_t &out) const;static bool parsePort_(JsonVariantConst v, uint8_t &out);static bool parseFloat_(JsonVariantConst v, float &out);static bool parseTarget_(JsonVariantConst v, int16_t &out);static Mode parseMode_(JsonVariantConst v);static const char *modeName_(Mode mode);bool setupButton_(const DeviceConfig &cfg, DeviceState &st);bool setupRelay_(const DeviceConfig &cfg, DeviceState &st);void handleButton_(const DeviceConfig &cfg, DeviceState &st);void updateControl_(const DeviceConfig &cfg, DeviceState &st);bool writeOff_(const DeviceConfig &cfg, DeviceState &st);bool writeOutputs_(const DeviceConfig &cfg, DeviceState &st, bool heat, bool cool);void notifyEvent_(uint8_t id, const String &name, bool power_on, const char *source);};

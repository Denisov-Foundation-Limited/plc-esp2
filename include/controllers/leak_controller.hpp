...
public:
    ...
    bool begin();
    void task();
    void applyConfig(JsonArrayConst zones);
    ...
    bool setSensorActiveLow(size_t id, bool active_low);
    bool setValveOpenOnPower(size_t id, bool open_on_power);
    bool setName(size_t id, const String &name);
    bool setSensorPort(size_t id, uint8_t port);
    bool setValvePort(size_t id, uint8_t port);
    bool setAlarmPort(size_t id, uint8_t port);
    bool ack(size_t id);
    bool ackAll();
    void setEventHandler(EventHandler cb, void *ctx);
    const ZoneConfig *config(size_t id) const;
    const ZoneState *state(size_t id) const;
    const ZoneConfig *configByIndex(size_t idx) const;
    const ZoneState *stateByIndex(size_t idx) const;
...
private:
    ...
    static bool indexById_(size_t id, size_t &out);
    bool setPort_(size_t id, uint8_t port, uint8_t ZoneConfig::*field, bool input);
    void setupZone_(const ZoneConfig &cfg, ZoneState &st);
    void setupInput_(uint8_t port);
    void setupOutput_(uint8_t port);
    bool readSensor_(const ZoneConfig &cfg, bool &wet);
    void writeOutputs_(const ZoneConfig &cfg, ZoneState &st, bool alarm);
    void notifyLeak_(const ZoneConfig &cfg);
    void notifyEvent_(Event ev, uint8_t id, const String &name, bool wet, bool alarm_latched);
    static constexpr uint32_t kDetectEventDebounceMs = 10000;
};

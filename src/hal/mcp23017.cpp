...
bool Mcp23017::begin(TwoWire &wire, uint8_t addr)
{
    ...
    _err = Error::Ok;
    _cache_valid = false;
    return cacheFromDevice_();
}

Mcp23017::Error Mcp23017::lastError() const { return _err; }
bool Mcp23017::ready() const { return _wire != nullptr; }
...
bool Mcp23017::readPort(Port port, uint8_t &out) const { return readGpio(port, out); }
bool Mcp23017::readGpio(Port port, uint8_t &out) const { return getPortReg_(REG_GPIOA, port, out); }
bool Mcp23017::readOlat(Port port, uint8_t &out) const { return getPortReg_(REG_OLATA, port, out); }
...
bool Mcp23017::readInterruptFlags(Port port, uint8_t &out) const { return getPortReg_(REG_INTFA, port, out); }
bool Mcp23017::readInterruptCapture(Port port, uint8_t &out) const { return getPortReg_(REG_INTCAPA, port, out); }
...
bool Mcp23017::cacheFromDevice_()
{
    ...
    uint8_t v = 0;
    if (!readReg(REG_IODIRA, v)) return false; _iodir[0] = v;
    if (!readReg(REG_IODIRB, v)) return false; _iodir[1] = v;
    if (!readReg(REG_IPOLA, v)) return false; _ipol[0] = v;
    if (!readReg(REG_IPOLB, v)) return false; _ipol[1] = v;
    if (!readReg(REG_GPINTENA, v)) return false; _gpinten[0] = v;
    if (!readReg(REG_GPINTENB, v)) return false; _gpinten[1] = v;
    if (!readReg(REG_DEFVALA, v)) return false; _defval[0] = v;
    if (!readReg(REG_DEFVALB, v)) return false; _defval[1] = v;
    if (!readReg(REG_INTCONA, v)) return false; _intcon[0] = v;
    if (!readReg(REG_INTCONB, v)) return false; _intcon[1] = v;
    if (!readReg(REG_IOCON, v)) return false; _iocon = v;
    if (!readReg(REG_GPPUA, v)) return false; _gppu[0] = v;
    if (!readReg(REG_GPPUB, v)) return false; _gppu[1] = v;
    if (!readReg(REG_OLATA, v)) return false; _olat[0] = v;
    if (!readReg(REG_OLATB, v)) return false; _olat[1] = v;
...
{
    const uint8_t mask = (uint8_t)(1u << bit);
    if (value) reg |= mask; else reg &= (uint8_t)~mask;
    return true;
}
...
bool Mcp23017::regForPort_(uint8_t base_reg, Port port, uint8_t &out) const
{
    ...
    switch (base_reg)
    {
    case REG_IODIRA: out = (port == Port::A) ? 0x00 : 0x10; return true;
    case REG_IPOLA: out = (port == Port::A) ? 0x01 : 0x11; return true;
    case REG_GPINTENA: out = (port == Port::A) ? 0x02 : 0x12; return true;
    case REG_DEFVALA: out = (port == Port::A) ? 0x03 : 0x13; return true;
    case REG_INTCONA: out = (port == Port::A) ? 0x04 : 0x14; return true;
    case REG_IOCON: out = (port == Port::A) ? 0x05 : 0x15; return true;
    case REG_GPPUA: out = (port == Port::A) ? 0x06 : 0x16; return true;
    case REG_INTFA: out = (port == Port::A) ? 0x07 : 0x17; return true;
    case REG_INTCAPA: out = (port == Port::A) ? 0x08 : 0x18; return true;
    case REG_GPIOA: out = (port == Port::A) ? 0x09 : 0x19; return true;
    case REG_OLATA: out = (port == Port::A) ? 0x0A : 0x1A; return true;
    default:
        out = (uint8_t)(base_reg + (port == Port::B ? 1 : 0));
        return true;
    }
...
bool Mcp23017::isBanked_() const
{
    return (_iocon & 0x80u) != 0;
}

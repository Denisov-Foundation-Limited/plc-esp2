...
void App::updateStackMasterMode_(){
...
void App::updateSecurityAlarms_(){
...
bool App::requestStackPollFeature_(uint32_t node_id, uint8_t feature){
    switch (feature)
    {
    case 0:  return stack_cache.requestSockets(node_id);
    case 1:  return stack_cache.requestLights(node_id);
    case 2:  return stack_cache.requestSecurity(node_id);
    case 3:  return stack_cache.requestSecurityPrearm(node_id);
    case 4:  return stack_cache.requestThermo(node_id);
    case 5:  return stack_cache.requestSeptic(node_id);
    case 6:  return stack_cache.requestTanks(node_id);
    case 7:  return stack_cache.requestMeteo(node_id);
    case 8:  return stack_cache.requestWatering(node_id);
    case 9:  return stack_cache.requestAvr(node_id);
    case 10: return stack_cache.requestLeak(node_id);
    case 11: return stack_cache.requestPorts(node_id);
    case 12: return stack_cache.requestTempSensors(node_id);
    default: return false;
    }
}

bool App::bootstrapSyncCompleted_(uint32_t node_id) const{
    if (node_id == 0)
        return true;
...
void App::enqueueStackBootstrapSync_(uint32_t node_id){
    if (node_id == 0)
        return;
...
void App::removeStackBootstrapSync_(uint32_t node_id){
    if (node_id == 0)
        return;
    if (_stack_bootstrap_node_id == node_id)
    {
...
void App::tryStartNextStackBootstrapSync_(){
...
void App::startStackBootstrapSync_(uint32_t node_id){
    if (node_id == 0)
        return;
...
void App::stopStackBootstrapSync_(bool timeout){
...
bool App::onRemoteMeteo_(void *ctx, uint32_t node_id, uint8_t sensor_id, float &temp_c, bool &has_temp){
...
bool App::onRemoteMeteoProxy_(void *ctx, uint32_t node_id, uint8_t sensor_id,
...
bool App::onRemoteNodeName_(void *ctx, uint32_t node_id, String &out){
    if (!ctx || node_id == 0)
        return false;
    App *self = static_cast<App *>(ctx);
...
bool App::onRemoteSensorName_(void *ctx, uint32_t node_id, uint8_t sensor_id, String &out){
    if (!ctx || node_id == 0 || sensor_id == 0)
        return false;
    App *self = static_cast<App *>(ctx);
    if (self->stackMasterActive_())
...
bool App::onRemoteSensorType_(void *ctx, uint32_t node_id, uint8_t sensor_id,
...
void App::onMeteoAlarm_(void *ctx, uint32_t node_id, uint8_t sensor_id, bool alarm){
...
void App::onSecurityArmState_(void *ctx, bool armed){
    if (!ctx)
        return;
    App *self = static_cast<App *>(ctx);
...
bool App::onSecurityPreArmCheck_(void *ctx, String &out, String *plain_out){
    if (!ctx)
        return false;
    return static_cast<App *>(ctx)->collectRemoteSecurityDetections_(out, plain_out);
}

void App::onStackNodeEvent_(void *ctx, uint32_t node_id, bool online){
    if (!ctx || node_id == 0)
        return;
    App *self = static_cast<App *>(ctx);
...
void App::onSecurityAlarmState_(void *ctx, bool alarm_on){
    if (!ctx)
        return;
    static_cast<App *>(ctx)->broadcastSecurityAlarm_(alarm_on);
}

void App::onSecurityClearDetect_(void *ctx){
    if (!ctx)
        return;
    App *self = static_cast<App *>(ctx);
...
void App::onSecurityDetect_(void *ctx, uint8_t sensor_id, const String &name, bool silent){
    if (!ctx)
        return;
    App *self = static_cast<App *>(ctx);
...
void App::onSepticDetect_(void *ctx, uint8_t septic_id, const String &name, bool is_alarm){
    if (!ctx)
        return;
    App *self = static_cast<App *>(ctx);
...
void App::onTankEmpty_(void *ctx, uint8_t tank_id, const String &name, bool empty){
    if (!ctx)
        return;
    if (!empty)
        return;
...
void App::onWateringEvent_(void *ctx, WateringController::Event ev,
...
void App::onRingHold_(void *ctx, bool on){
    if (!ctx)
        return;
    App *self = static_cast<App *>(ctx);
...
void App::onStackFrame_(void *ctx, uint32_t node_id, const StackFrame &frame){
    if (!ctx || node_id == 0)
        return;
    static_cast<App *>(ctx)->handleStackFrame_(node_id, frame);
}

bool App::onSecurityRfidUid_(void *ctx, const String &uid){
    if (!ctx)
        return false;
    return static_cast<App *>(ctx)->handleSecurityRfidUid_(uid);
}

bool App::onSecurityIButtonSerial_(void *ctx, const String &serial){
    if (!ctx)
        return false;
    return static_cast<App *>(ctx)->handleSecurityIButtonSerial_(serial);
}

bool App::onDisplaySlot_(void *ctx, const DisplaySlotConfig &slot, char out[5]){
    if (!ctx)
        return false;
    return static_cast<App *>(ctx)->renderDisplaySlot_(slot, out);
}

void App::broadcastSecurityState_(bool armed){
    if (!stackMasterActive_())
        return;
    StackMaster &master = net.network.stackMaster();
    const size_t count = master.nodeCount();
...
void App::sendSecurityStateToNode_(uint32_t node_id){
    sendSecurityStateToNode_(node_id, control.controllers.security().armed(), false);
}

void App::sendSecurityStateToNode_(uint32_t node_id, bool armed, bool force){
    if (node_id == 0)
        return;
    if (!stackMasterActive_())
        return;
...
void App::sendSecurityDetectToMaster_(uint8_t sensor_id, const String &name, bool silent){
...
void App::sendSepticDetectToMaster_(uint8_t septic_id, const String &name, bool is_alarm){
    if (!stackSlaveActive_())
        return;
    StackNode &node = net.network.stackNode();
    if (!node.connected())
...
void App::sendTankEmptyToMaster_(uint8_t tank_id, const String &name){
    if (!stackSlaveActive_())
        return;
    StackNode &node = net.network.stackNode();
    if (!node.connected())
...
void App::sendWateringEventToMaster_(WateringController::Event ev,
                                const WateringController::RuleConfig &cfg,
                                const WateringController::RuleState &st){
    if (!stackSlaveActive_())
        return;
...
void App::broadcastRingHold_(bool on){
    if (!stackMasterActive_())
        return;
    StackMaster &master = net.network.stackMaster();
    const size_t count = master.nodeCount();
...
void App::sendTelegramNotify_(const String &msg){
...
void App::handleStackFrame_(uint32_t node_id, const StackFrame &frame){
    if (!stackMasterActive_())
        return;
...
void App::handleSepticFrame_(uint32_t node_id, const String &action, JsonVariantConst params){
    if (action != "level")
        return;
...
void App::handleTankFrame_(uint32_t node_id, const String &action, JsonVariantConst params){
    if (action != "empty")
        return;
...
void App::handleWateringFrame_(uint32_t node_id, const String &action, JsonVariantConst params){
    if (action != "event")
        return;
...
void App::flushPendingSecurityDetect_(){
    if (!_pending_detect)
        return;
    if (!stackSlaveActive_())
        return;
...
void App::flushPendingSepticDetect_(){
...
void App::flushPendingTankEmpty_(){
...
void App::flushPendingWateringEvent_(){
...
void App::flushPendingRingButton_(){
...
bool App::handleSecurityRfidUid_(const String &uid_str){
    if (!stackSlaveActive_())
        return false;
    if (uid_str.length() == 0)
        return false;
...
bool App::handleSecurityIButtonSerial_(const String &serial){
    if (!stackSlaveActive_())
        return false;
    if (serial.length() == 0)
        return false;
...
bool App::sendRingButtonToMaster_(bool pressed){
    if (!stackSlaveActive_())
        return false;
    StackNode &node = net.network.stackNode();
    if (!node.connected())
...
bool App::renderDisplaySlot_(const DisplaySlotConfig &slot, char out[5]){
    if (!out)
        return false;
...
void App::formatTemp3_(char out[5], int t){
    if (!out)
        return;
    if (t <= -10)
        snprintf(out, 5, "%3d", t);
...
bool App::displaySlotEqual_(const DisplaySlotConfig &a, const DisplaySlotConfig &b){
    if (a.kind != b.kind || a.node_id != b.node_id || a.index != b.index || a.field != b.field)
        return false;
    return strncmp(a.text, b.text, sizeof(a.text)) == 0;
}

bool App::sendRfidToMaster_(const String &uid, const String &name){
    if (!stackSlaveActive_())
        return false;
    StackNode &node = net.network.stackNode();
    if (!node.connected())
...
bool App::sendIButtonToMaster_(const String &serial, const String &name){
    if (!stackSlaveActive_())
        return false;
    StackNode &node = net.network.stackNode();
    if (!node.connected())
...
void App::sendRfidResultToNode_(uint32_t node_id, const String &uid, bool matched,
...
void App::sendIButtonResultToNode_(uint32_t node_id, const String &serial, bool matched,
                              const String &result, bool armed){
    if (node_id == 0)
        return;
    if (!stackMasterActive_())
...
void App::pollSecurityStatusFromMaster_(){
    if (!stackSlaveActive_())
        return;
    StackNode &node = net.network.stackNode();
    if (!node.connected())
...
bool App::collectRemoteSecurityDetections_(String &out, String *plain_out){
    if (!stackMasterActive_())
        return false;
    StackMaster &master = net.network.stackMaster();
    const size_t count = master.nodeCount();
...
String App::stackNodeLabel_(uint32_t node_id) const{
...
String App::escapeHtml_(const String &in){
    String out;
...
int App::stackNodeIndex_(uint32_t node_id) const{
    StackMaster &master = const_cast<App *>(this)->net.network.stackMaster();
    const size_t count = master.nodeCount();
    for (size_t i = 0; i < count; ++i)
    {
...
void App::logStackNodeInventory_(uint32_t node_id){
...
App::StackInventoryLogState *App::inventoryLogState_(uint32_t node_id, bool create){
    if (node_id == 0)
        return nullptr;
...
void App::clearInventoryLogState_(uint32_t node_id){
    if (node_id == 0)
        return;
    for (size_t i = 0; i < StackMaster::MAX_SESSIONS; ++i)
    {
...
void App::broadcastSecurityAlarm_(bool alarm_on){
    if (!stackMasterActive_())
        return;
    StackMaster &master = net.network.stackMaster();
    const size_t count = master.nodeCount();
...
void App::sendSecurityAlarmToNode_(uint32_t node_id, bool alarm_on){
    if (node_id == 0)
        return;
    if (!stackMasterActive_())
        return;
...
void App::sendSecurityBeepToNode_(uint32_t node_id, const char *kind){
    if (node_id == 0)
        return;
    if (!stackMasterActive_())
        return;
...
void App::pollSecurityPrearmWarmup_(){
    if (!stackMasterActive_())
        return;
    const uint32_t now = millis();
...
void App::broadcastSecurityClear_(){
    if (!stackMasterActive_())
        return;
    StackMaster &master = net.network.stackMaster();
    const size_t count = master.nodeCount();
...
void App::sendSecurityClearToNode_(uint32_t node_id){
    if (node_id == 0)
        return;
    if (!stackMasterActive_())
        return;

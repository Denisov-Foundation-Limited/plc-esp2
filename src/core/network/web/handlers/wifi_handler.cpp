...
void WifiHandler::handleWifi(WebInterface &web, AsyncWebServerRequest *request) {
...
const char * WifiHandler::signalLabel_(int rssi) {
        if (rssi >= 20)
            return "best";
        if (rssi >= 14)
            return "medium";
...
String WifiHandler::formatSignalForWeb_(const String &signal) {
        String v = signal;
        v.trim();
        if (!v.length())
            return "";

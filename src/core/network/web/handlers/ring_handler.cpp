...
void RingHandler::registerRoutes(WebInterface &web, AsyncWebServer &server) {
...
void RingHandler::handleRing(WebInterface &web, AsyncWebServerRequest *request) {
...
void RingHandler::handleRingSave(WebInterface &web, AsyncWebServerRequest *request) {
        bool set_cookie = false;
        if (!web.checkAuth_(request, &set_cookie))
            return;
...
void RingHandler::handleRingTrigger(WebInterface &web, AsyncWebServerRequest *request) {
        bool set_cookie = false;
        if (!web.checkAuth_(request, &set_cookie))
            return;
        const uint32_t node_id = web.parseStackNodeIdParam_(request);

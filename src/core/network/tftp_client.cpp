...
bool TftpClient::download(const IPAddress &server, const String &path, WriteHandler writer, void *ctx,
              uint16_t port, uint16_t timeout_ms, uint8_t max_retries)
{
    _last_error = "";
    String file = path;
    if (file.startsWith("/"))
...
const String &TftpClient::lastError() const
{ return _last_error; }
bool TftpClient::sendRrq_(WiFiUDP &udp, const IPAddress &server, uint16_t port, const String &file)
{
...
void TftpClient::sendAck_(WiFiUDP &udp, const IPAddress &server, uint16_t port, uint16_t block)
{
    uint8_t pkt[4];
    pkt[0] = 0;
    pkt[1] = 4;
...
int TftpClient::readPacket_(WiFiUDP &udp, uint8_t *buf, size_t buf_len, uint16_t timeout_ms)
{

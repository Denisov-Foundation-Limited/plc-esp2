...
bool TelegramMenuWatering::cmdWatering_(TelegramBot &bot, const TelegramClient::Update &u, String &reply)
    {
        (void)reply;
    ...
    bool TelegramMenuWatering::cmdWateringList_(TelegramBot &bot, const TelegramClient::Update &u, String &reply)
    {
        if (!TelegramMenu::_self)
            return false;
        if (!TelegramMenu::requireAdmin_(*TelegramMenu::_self, bot, u, reply))
    ...
    bool TelegramMenuWatering::cmdWateringShow_(TelegramBot &bot, const TelegramClient::Update &u, String &reply)
    {
        if (!TelegramMenu::_self)
            return false;
        if (!TelegramMenu::requireAdmin_(*TelegramMenu::_self, bot, u, reply))
    ...
    void TelegramMenuWatering::buildWateringLabels_(TelegramMenu &self, int64_t chat_id, std::vector<String> &out)
    {
        out.clear();
    ...
    String TelegramMenuWatering::wateringControlMarkup_(TelegramMenu &self, int64_t chat_id)
    {
    ...
    String TelegramMenuWatering::wateringListTextHtml_(TelegramMenu &self, int64_t chat_id)
    {
    ...
    String TelegramMenuWatering::wateringRuleTextHtml_(TelegramMenu &self, int64_t chat_id, uint8_t id)
    {
        if (self.isLocalSelected_(chat_id))
        {
            if (!self._watering)
    ...
    String TelegramMenuWatering::wateringRuleControlMarkup_()
    {
        std::vector<String> labels;
        labels.reserve(3);
    ...
    void TelegramMenuWatering::sendWateringMenu_(TelegramMenu &self, int64_t chat_id)
    {
        if (!self._bot)
            return;
    ...
    void TelegramMenuWatering::sendWateringRule_(TelegramMenu &self, int64_t chat_id, uint8_t id)
    {
        if (!self._bot)
            return;
        if (self.isLocalSelected_(chat_id) && !self._watering)
    ...
    bool TelegramMenuWatering::handleWateringAction_(TelegramMenu &self, const TelegramClient::Update &u)
    {
    ...
    bool TelegramMenuWatering::handleWateringSelection_(TelegramMenu &self, const TelegramClient::Update &u)
    {
        if (!self._bot)
            return false;
    ...
    bool TelegramMenuWatering::parseWateringIdFromText_(const String &text, uint8_t &out)
    {
    ...
    bool TelegramMenuWatering::parseWateringId_(const String &text, uint8_t &out)
    {
        String t = text;
        t.trim();
        if (t.length() == 0)
    ...
    bool TelegramMenuWatering::parseWateringLabel_(const String &text, uint8_t &out)
    {
        String t = text;
        t.trim();
        if (t.length() == 0)

...
bool TelegramMenuLeak::cmdLeak_(TelegramBot &bot, const TelegramClient::Update &u, String &reply)
    {
        (void)reply;
    ...
    bool TelegramMenuLeak::cmdLeakList_(TelegramBot &bot, const TelegramClient::Update &u, String &reply)
    {
        if (!TelegramMenu::_self)
            return false;
        if (!TelegramMenu::requireAdmin_(*TelegramMenu::_self, bot, u, reply))
    ...
    bool TelegramMenuLeak::cmdLeakShow_(TelegramBot &bot, const TelegramClient::Update &u, String &reply)
    {
        (void)bot;
        if (!TelegramMenu::_self)
            return false;
    ...
    bool TelegramMenuLeak::cmdLeakAck_(TelegramBot &bot, const TelegramClient::Update &u, String &reply)
    {
        (void)bot;
        if (!TelegramMenu::_self)
            return false;
    ...
    void TelegramMenuLeak::buildLeakLabels_(TelegramMenu &self, int64_t chat_id, std::vector<String> &out)
    {
        out.clear();
    ...
    String TelegramMenuLeak::leakListTextHtml_(TelegramMenu &self, int64_t chat_id)
    {
    ...
    String TelegramMenuLeak::leakZoneTextHtml_(TelegramMenu &self, int64_t chat_id, uint8_t id)
    {
        if (self.isLocalSelected_(chat_id))
        {
            if (!self._leak)
    ...
    String TelegramMenuLeak::leakZoneControlMarkup_()
    {
    ...
    void TelegramMenuLeak::sendLeakMenu_(TelegramMenu &self, int64_t chat_id)
    {
        if (!self._bot)
            return;
    ...
    void TelegramMenuLeak::sendLeakZone_(TelegramMenu &self, int64_t chat_id, uint8_t id)
    {
        if (!self._bot)
            return;
        if (self.isLocalSelected_(chat_id) && !self._leak)
    ...
    bool TelegramMenuLeak::handleLeakAction_(TelegramMenu &self, const TelegramClient::Update &u)
    {
    ...
    bool TelegramMenuLeak::handleLeakSelection_(TelegramMenu &self, const TelegramClient::Update &u)
    {
        if (!self._bot)
            return false;
    ...
    bool TelegramMenuLeak::parseLeakIdFromText_(const String &text, uint8_t &out)
    {
    ...
    bool TelegramMenuLeak::parseLeakId_(const String &text, uint8_t &out)
    {
        String t = text;
        t.trim();
        if (t.length() == 0)
    ...
    bool TelegramMenuLeak::parseLeakLabel_(const String &text, uint8_t &out)
    {
        String t = text;
        t.trim();
        if (t.length() == 0)

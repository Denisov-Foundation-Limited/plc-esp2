...
void RulesController::resetDefaults()
{
    if (!_rules)
        return;
...
bool RulesController::takeDirty()
{
    if (!_dirty)
        return false;
    _dirty = false;
...
RulesController::Rule *RulesController::rule(size_t id)
{
    if (!_rules || id == 0 || id > kRuleCount)
        return nullptr;
    return &_rules[id - 1];
}
const RulesController::Rule *RulesController::rule(size_t id) const
{
    if (!_rules || id == 0 || id > kRuleCount)
        return nullptr;
    return &_rules[id - 1];
}
const RulesController::Rule *RulesController::ruleByIndex(size_t idx) const
{
    if (!_rules || idx >= kRuleCount)
        return nullptr;
    return &_rules[idx];
}
RulesController::RuleAction *RulesController::action(size_t rule_id, size_t action_id)
{
    Rule *r = rule(rule_id);
...
const RulesController::RuleAction *RulesController::action(size_t rule_id, size_t action_id) const
{
...
void RulesController::applyConfig(JsonArrayConst arr)
{
    if (!_rules)
        return;
    resetDefaults();
...
void RulesController::serialize(JsonArray out) const
{
    if (!_rules)
        return;
    for (size_t i = 0; i < kRuleCount; ++i)
...
{ return _last_triggered_rule_id; }
uint32_t RulesController::lastTriggeredMs() const
{ return _last_triggered_ms; }
void RulesController::initStorage_()
{
...
void RulesController::releaseStorage_()
{
    if (!_rules)
        return;
    for (size_t i = 0; i < kRuleCount; ++i)

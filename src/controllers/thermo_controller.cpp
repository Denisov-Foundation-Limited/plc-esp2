...
ThermoController::ThermoController(Gpio &gpio, MeteoController &meteo, Logger &logs)
...
bool ThermoController::indexById_(uint8_t id, size_t &out) const{
...
bool ThermoController::parsePort_(JsonVariantConst v, uint8_t &out){
    if (v.is<unsigned>())
    {
...
bool ThermoController::parseFloat_(JsonVariantConst v, float &out){
...
ThermoController::Mode ThermoController::parseMode_(JsonVariantConst v){
    if (v.is<unsigned>())
    {
...
const char *ThermoController::modeName_(ThermoController::Mode mode){
    switch (mode)
    {
    case Mode::Heat:
        return "heat_only";
...
bool ThermoController::setupButton_(const ThermoController::DeviceConfig &cfg, ThermoController::DeviceState &st){
    if (cfg.button_port == kInvalidPort)
        return false;
...
bool ThermoController::setupRelay_(const ThermoController::DeviceConfig &cfg, ThermoController::DeviceState &st){
    bool ok = true;
    if (cfg.heat_port != kInvalidPort)
    {
...
void ThermoController::handleButton_(const ThermoController::DeviceConfig &cfg, ThermoController::DeviceState &st){
    bool raw = false;
    if (!_gpio.readDyn(cfg.button_port, raw))
        return;
...
void ThermoController::updateControl_(const ThermoController::DeviceConfig &cfg, ThermoController::DeviceState &st){
...
bool ThermoController::writeOff_(const ThermoController::DeviceConfig &cfg, ThermoController::DeviceState &st){
    return writeOutputs_(cfg, st, false, false);
}

bool ThermoController::writeOutputs_(const ThermoController::DeviceConfig &cfg, ThermoController::DeviceState &st, bool heat, bool cool){
    bool changed = false;
    if (heat && cool)
        cool = false;

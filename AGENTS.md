...
- `Extender`:
  ...
  - нельзя кодировать read-fail как обычный логический `false`: путь `Extender::read() -> PortIO::read() -> IoStack::scanInputs()` обязан различать `уровень LOW` и `чтение не удалось`, иначе краткий lock timeout / not-ready / I2C коллизия на общей шине превращается в ложный `LOW` на входе и далее в флаппинг/ложный detect. При fail нужно возвращать отдельный статус ошибки и по возможности сохранять последнее валидное cached значение, не обновляя вход новым нулём.
- `PortIO`:
